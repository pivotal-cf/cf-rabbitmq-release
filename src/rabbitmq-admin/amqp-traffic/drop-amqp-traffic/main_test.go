...
var _ = Describe("Main", func() {
	...
	It("prints a message saying what it is going to do", func() {
		...
		Eventually(session.Out).Should(Say("The following commands will be used to block AMQP and AMQPS traffic on this node"))
	})

	It("prints the commands that it is going to run", func() {
		session := runWithInput(pathToCmd, "no\n", 0)

		Eventually(session.Out).Should(Say("iptables -A INPUT -p tcp --dport 5671 -j DROP"))
