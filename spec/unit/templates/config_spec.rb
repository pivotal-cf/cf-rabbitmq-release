  ...
  context 'internode TLS is enabled' do
    ...
    context 'TLS is disabled' do
      ...
      it 'does contain erl args for rabbitmqctl' do
        expect(rendered_template).to include('CTL_ERL_ARGS="-pa $ERL_SSL_PATH -proto_dist inet_tls -ssl_dist_optfile /var/vcap/jobs/rabbitmq-server/etc/inter_node_tls.config"')
      end
    end
