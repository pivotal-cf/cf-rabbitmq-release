...
RSpec.describe "haproxy" do
  ...
  [0, 1].each do |job_index|
    context "when the job rabbitmq-server/#{job_index} is down", :pushes_cf_app do
      before(:all) do
        `bosh -n stop rabbitmq-server #{job_index} --force`
      end

      after(:all) do
        `bosh -n start rabbitmq-server #{job_index} --force`
      end
