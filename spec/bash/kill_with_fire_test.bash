...
T_should_kill_a_pid() {
  (
    ...
    ) || ( $T_fail "Process still existed after running the kill script" && return 1 )
}
...
T_should_exit_non_zero_if_not_provided_a_pid_file() {
  (
    ...
    ) || ( $T_fail "Exited zero when not provided a PID" && return 1 )
}
...
T_should_log_if_not_provided_a_pid_file() {
  (
    ...
    ) || ( $T_fail "Did not provide the correct output when no PID is provided" && return 1 )
}
...
T_should_warn_the_user_if_provided_a_non_existent_pid_file() {
  (
    ...
    ) || ( $T_fail "Did not output that it's continuing after PID file was not found" && return 1 )
}

T_should_exit_zero_if_provided_a_non_existent_pid_file() {
  (
    run_the_kill_script /path/does/not/exist
    ) || ( $T_fail "Exited non zero when provided a non existent PID file" && return 1 )
}
...
T_should_do_nothing_when_the_pid_does_not_exist() {
  (
    ...
    run_the_kill_script "$PID_FILE"
    ) || ( $T_fail "Should exit 0 when the given PID does not exist" && return 1 )
}
...
T_should_remove_pid_file() {
  (
    ...
    [[ ! -e "$PID_FILE" ]]
    ) || ( $T_fail "PID_FILE should be deleted after the process is killed" && return 1 )
}
...
T_should_log_a_message_if_a_process_existed() {
  (
    ...
    ) || ( $T_fail "Should log message when a process is killed" && return 1 )
}
